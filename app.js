...
async function barberSignup() {
    ...
    if (!name || !phone || !password || !confirmPassword || !photoFile) {
    ...
    try {
        ...
        const photoReference = storageRef(storage, `barberPhotos/${user.uid}`);
        await uploadBytes(photoReference, photoFile);
        ...
        // تحديث صورة الحلاق في لوحة التحكم
        document.getElementById('barberAvatarImg').src = photoURL;
        showBarberDashboard();
        loadBarberQueue();
...
async function barberLogin() {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            // تحديث صورة الحلاق في لوحة التحكم
            document.getElementById('barberAvatarImg').src = barberData.photoURL;
            showBarberDashboard();
            loadBarberQueue();
        } else {
            ...
            barberCard.innerHTML = `
                <div class="barber-info">
                    <div class="barber-header">
                        ${barber.photoURL ? 
                            `<img src="${barber.photoURL}" class="barber-avatar" alt="${barber.name}">` : 
...
async function logout() {
    try {
        ...
        document.getElementById('confirmBarber
